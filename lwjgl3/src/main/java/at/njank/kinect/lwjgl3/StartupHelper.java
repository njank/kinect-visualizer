...
public class StartupHelper {
	...
	public static boolean startNewJvmIfRequired(boolean inheritIO) {
		if(!System.getProperty("os.name").toLowerCase(Locale.ROOT).contains("windows")){
            throw new UnsupportedOperationException("This program is only supported on Windows systems.");
        }

        try {
            NativeLoader.load("ufdw_j4k"+ (System.getProperty("os.arch").contains("64") ? "_64bit" : "_32bit"));
        } catch(UnsatisfiedLinkError e) {
            System.out.println("Kinect SDK1 not installed.");
        }
        try {
            NativeLoader.load("ufdw_j4k2"+ (System.getProperty("os.arch").contains("64") ? "_64bit" : "_32bit"));
        } catch(UnsatisfiedLinkError e) {
            System.out.println("Kinect SDK2 not installed.");
        }
